...
const app = express();
const PORT = process.env.PROFILE_RELAYER_PORT || 3002;
...
const ProfileRelayRequestSchema = z.object({
...
app.get('/health', (req, res) => {
  res.json({ 
    ...
    profileSBT: PROFILE_SBT_ADDRESS,
    timestamp: new Date().toISOString()
  });
});

// Profile relay endpoint
app.post('/api/profile/relay', verifyHMAC, async (req, res) => {
  try {
    // Validate request body
    const { request, chainId, forwarder, signature } = ProfileRelayRequestSchema.parse(req.body);
    ...
    console.log(`Profile transaction successful: ${receipt.hash}`, {
      from: request.from,
      to: request.to,
    ...
    console.error('Profile relay error:', error);
...
// Start server
app.listen(PORT, () => {
  console.log(`Profile relayer server running on port ${PORT}`);
  console.log(`Chain ID: ${CHAIN_ID}`);
  console.log(`Forwarder: ${FORWARDER_ADDRESS}`);
  console.log(`ProfileSBT: ${PROFILE_SBT_ADDRESS}`);
